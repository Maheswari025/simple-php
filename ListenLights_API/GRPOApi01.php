...
if($class == "GOODSISSUE1"){
class GOODSISSUE1 extends connection{
	 function getGRPOlist(){
		 $conn=$this->getconnection();
		//$t=   $_REQUEST['region'];
		//$q=   $_REQUEST['search'] ;
		...
		$ProjectCode=$_REQUEST['ProjectCode'] ;
		//$BPL=$_REQUEST['BPLName'] ;
		$qry = "SELECT A.DocNum,A.DOCENTRY,d.PrjCode, A.DocDate 'Posting Date',a.TaxDate 'Document Date',a.BPLName,a.Series,a.DocTotal
FROM OIGE A
INNER JOIN IGE1 B ON A.DOCENTRY=B.DocEntry
left outer join OBPL c ON C.BPLId=A.BPLId
LEFT OUTER JOIN OPRJ D ON b.Project = d.PrjCode 
where D.PrjCode='$ProjectCode' AND A.DocDate BETWEEN '$fd' AND '$td'";
		...
		$json_response = array();
			while($fetch = odbc_fetch_array($process)){
				$LoadData[]  = $fetch;
				$row_array = $fetch;
				$DOC_NO = utf8_encode($fetch['PrjCode']);   
//echo $DOC_NO ;
		$row_array['LineItems']= array();
		
		$qry1 = "SELECT B.DOCENTRY,b.ItemCode,
d.PrjCode,b.Dscription 'Item Description',b.LineTotal,b.Quantity,b.Price,(b.Quantity*b.Price)'Total',b.WhsCode,b.unitMsr'Nos'
FROM OIGE A
INNER JOIN IGE1 B ON A.DOCENTRY=B.DOCENTRY
left outer join OBPL c ON C.BPLId=A.BPLId
LEFT OUTER JOIN OPRJ D ON D.PrjCode= b.Project
where  A.DocDate BETWEEN '$fd' AND '$td' And D.PrjCode ='$DOC_NO'";
		 ...
		 $process1 = odbc_exec($conn,$qry1);
			while($fetch1 = odbc_fetch_array($process1)){
				$row_array['LineItems'][] = array(
					'ProjectCode' => utf8_encode($fetch1['PrjCode']),
					'Item Code' => utf8_encode($fetch1['ItemCode']),
				    'Item Description' => utf8_encode($fetch1['Item Description']),
					'Quantity' => utf8_encode($fetch1['Quantity']),
				    'WhsCode' => utf8_encode($fetch1['WhsCode']),
					'LineTotal' => utf8_encode($fetch1['LineTotal']),
				    'UOM' => utf8_encode($fetch1['Nos'])
